...
get data() {
if (!this._data) {
try { const s = localStorage.getItem(“rennesmoov_v3”); this._data = s ? {…this._defaults,…JSON.parse(s)} : {…this._defaults}; }
...
document.getElementById("modal-trail-content").innerHTML = `
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:18px;">
    ${[
      {v: t.duration + " min", l: "Durée"},
      {v: t.distance + " km", l: "Distance"},
      {v: "+" + t.elevation + "m", l: "D+"},
      {v: t.kcal, l: "Kcal"}
    ].map(s => `
      <div style="background:var(--surface-soft);border-radius:var(--r-md);padding:10px;text-align:center;border:1px solid var(--border-subtle);">
        <div style="font-family:'Fraunces',serif;font-size:17px;font-weight:500;color:var(--ink-950);letter-spacing:-0.03em;font-variant-numeric:tabular-nums;">${s.v}</div>
        <div style="font-size:10px;color:var(--text-secondary);font-weight:600;margin-top:3px;text-transform:uppercase;letter-spacing:0.06em;">${s.l}</div>
      </div>`).join("")}
  </div>
...
// Inject GPS + mini map + launch button
setTimeout(() => {
...
document.getElementById("modal-trail-content").innerHTML = `
  ...
  ${organizer ? `<div style="background:var(--surface-soft);border-radius:var(--r-md);padding:14px;margin-bottom:16px;border:1px solid var(--border-subtle);">
    <div style="font-size:10.5px;font-weight:700;color:var(--text-secondary);text-transform:uppercase;letter-spacing:0.08em;margin-bottom:10px;">Organisateur</div>
    <div style="display:flex;align-items:center;gap:12px;">
      <img src="${organizer.img}" style="width:40px;height:40px;border-radius:50%;border:2px solid ${organizer.color};object-fit:cover;" onerror="this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(organizer.name)}&background=${organizer.color.replace('#','')}&color=fff&bold=true&size=80'">
      <div><div style="font-size:13.5px;font-weight:700;color:var(--ink-950);letter-spacing:-0.005em;">${organizer.name}</div><div style="font-size:11.5px;color:var(--text-secondary);font-weight:500;">${organizer.handle}</div></div>
    </div>
  </div>` : ""}
...
grid.innerHTML = DB.badges.map(b => ` <div class="badge-item ${b.unlocked ? "" : "locked"}" onclick="Toast.show('${b.name}','${b.desc}','green','${b.unlocked?"medal":"lock"}')"> <span class="badge-icon">${b.icon}</span> <span class="badge-name">${b.name}</span> </div>`).join(””);
},
...
const hint = hints.find((*, i) => !checks[i]) || “Profil complet !”;
...
/* =============================================================================
TRAIL SESSION (fullscreen run mode)
============================================================================= */
...
document.getElementById("modal-session-summary-content").innerHTML = `
  <div style="text-align:center;padding:8px 0 20px;">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:18px;">
      <div style="background:var(--surface-soft);border-radius:var(--r-lg);padding:14px;border:1px solid var(--border-subtle);">
        <div style="font-family:'Fraunces',serif;font-size:22px;font-weight:500;color:var(--ink-950);letter-spacing:-0.03em;font-variant-numeric:tabular-nums;">${m}:${s.toString().padStart(2,"0")}</div>
        <div style="font-size:10px;color:var(--text-secondary);font-weight:700;text-transform:uppercase;letter-spacing:0.08em;margin-top:4px;">Temps</div>
      </div>
      <div style="background:var(--surface-soft);border-radius:var(--r-lg);padding:14px;border:1px solid var(--border-subtle);">
        <div style="font-family:'Fraunces',serif;font-size:22px;font-weight:500;color:var(--ink-950);letter-spacing:-0.03em;font-variant-numeric:tabular-nums;">${this._distKm.toFixed(2)}</div>
        <div style="font-size:10px;color:var(--text-secondary);font-weight:700;text-transform:uppercase;letter-spacing:0.08em;margin-top:4px;">km</div>
      </div>
      <div style="background:var(--energy-100);border-radius:var(--r-lg);padding:14px;border:1px solid var(--energy-200);">
        <div style="font-family:'Fraunces',serif;font-size:22px;font-weight:500;color:var(--energy-600);letter-spacing:-0.03em;font-variant-numeric:tabular-nums;">+${xp}</div>
        <div style="font-size:10px;color:var(--text-secondary);font-weight:700;text-transform:uppercase;letter-spacing:0.08em;margin-top:4px;">XP</div>
      </div>
    </div>
...
delete Store.data.myProfile.*pendingImg;
}
Store.data.myProfile.name = name;
Store.data.myProfile.sport = sport;
Store.data.myProfile.handle = “@” + name.toLowerCase().replace(/\s+/g, “*”);
Store.save();
Modal.closeAll();
ProfileView.render();
...
```
// Show auth screen on first launch (no saved data)
...
/* =============================================================================
GLOBAL EXPORTS (pour les onclick inline)
============================================================================= */
window.App = App;
window.Modal = Modal;
window.Toast = Toast;
